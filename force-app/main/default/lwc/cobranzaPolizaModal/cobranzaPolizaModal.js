...
export default class CobranzaPolizaModal extends LightningModal {
    ...
    formatDate(dateString) {
        ...
        try {
            console.log('dateString en formatDate EN MODAL:', dateString);

            const [year, month, day] = dateString.split('-').map(Number);
            const date = new Date(year, month - 1, day);

            console.log('date corregida:', date);
    ...
    get diasVencimientoHoy() {
        if(this.pago.Estatus_pago__c === 'Pendiente' || this.pago.Estatus_pago__c === 'Vencida') {
            const hoy = new Date();
