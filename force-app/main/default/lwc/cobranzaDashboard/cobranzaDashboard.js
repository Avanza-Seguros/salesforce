...
const ASEGURADORA_OPCIONES = [
    ...
    {label:'PLAN SEGURO',value:'PLAN SEGURO'},
...
export default class CobranzaDashboard extends LightningElement {
    ...
    async abrirModalDetalles(pago) {
        ...
        if (!pago.Poliza__c) {
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Pago sin póliza asociada',
                    message: 'Este pago no tiene una póliza asociada, por lo tanto no es posible ver el detalle.',
                    variant: 'warning'
                })
            );
            return;
    ...
    prepararDatosBarrasApiladas(data) {
        ...
        grupos.forEach(grupo => {
            ...
            const vencido = grupo.estatus.find(e => e.estatus === 'Vencida');
    ...
    async filtrarPagos() {
        ...
        if (this.filtros.sinPoliza) {
            resultados = resultados.filter(pago => !pago.Poliza__c);
        }
    ...
    */
    ...
    actualizarDatosPagina() {
        ...
        this.paginatedData = this.filteredPagos
    ...
    get fechaDesdeFormateada() {
        if (!this.filtros.fechaDesde) return '';
        return this.formatDateLocal(this.filtros.fechaDesde);
    }

    get fechaHastaFormateada() {
        if (!this.filtros.fechaHasta) return '';
        return this.formatDateLocal(this.filtros.fechaHasta);
    }

    get graficaClass() {
        return this.showGrafica ? 'grafica-visible' : 'grafica-oculta';
    }

    get textoBotonGrafica() {
        return this.showGrafica ? 'Ocultar Gráfica' : 'Ver Gráfica';
    }
    ...
    sortFilteredPagos() {
        ...
        this.filteredPagos = [...this.filteredPagos].sort((a, b) => {
            let valueA = a[field];
            let valueB = b[field];
            ...
            if (
                field === 'Fecha_de_Aplicaci_n_del_pago__c' ||
                field === 'Proximo_Pago__c'
            ) {
                return (new Date(valueA) - new Date(valueB)) * direction;
            }

            if (field === 'Monto__c' || field === 'Pago_en_MXN__c') {
    ...
    handleToggleGrafica() {
        this.showGrafica = !this.showGrafica;
    }
